...
{
	return ImGui::GetCurrentContext() ? (ImGui_ImplOSPP_Data*)ImGui::GetIO().BackendPlatformUserData
									  : nullptr;
}
...
auto ImGui_ImplOSPP_ProcessEvent(const os::event* event) -> bool
{
	...
	switch(event->type)
	{
		...
		case os::events::mouse_button:
		{
			...
			bd->MouseButtonsDown = pressed ? (bd->MouseButtonsDown | (1 << mouse_button))
			...
			switch(event->window.type)
			{
				...
				case os::window_event_id::resized:
					if(ImGuiViewport* viewport =
						   ImGui_ImplOSPP_GetViewportFromWindowId(event->window.window_id))
					{
...
auto ImGui_ImplOSPP_Init(render_window* window, ImGui_ImplOSPP_RenderWindow_Callback render_callback,
...
static void ImGui_ImplOSPP_CreateWindow(ImGuiViewport* viewport)
{
	...
	win_flags |=
		(viewport->Flags & ImGuiViewportFlags_NoDecoration) ? os::window::borderless : os::window::resizable;
	...
	os::window os_win("No Title Yet", int32_t(viewport->Pos.x), int32_t(viewport->Pos.y),
					  uint32_t(viewport->Size.x), uint32_t(viewport->Size.y), win_flags);
