...
//        io.ConfigViewportsNoTaskBarIcon = true;
		...
		m_program = gfx::create_program(
			...
			gfx::create_embedded_shader(s_embeddedShaders, type, "fs_ocornut_imgui"), true);

		u_imageLodEnabled = gfx::create_uniform("u_imageLodEnabled", gfx::uniform_type::Vec4);
		m_imageProgram =
			gfx::create_program(gfx::create_embedded_shader(s_embeddedShaders, type, "vs_imgui_image"),
								gfx::create_embedded_shader(s_embeddedShaders, type, "fs_imgui_image"), true);
		...
		int32_t height{};
		{
			...
			m_font[ImGui::Font::Mono] =
				io.Fonts->AddFontFromMemoryTTF((void*)s_robotoMonoRegularTtf, sizeof(s_robotoMonoRegularTtf),
			...
			for(uint32_t ii = 0; ii < BX_COUNTOF(s_fontRangeMerge); ++ii)
			{
				...
				io.Fonts->AddFontFromMemoryTTF((void*)frm.data, (int)frm.size, _fontSize - 3.0f, &config,
											   frm.ranges);
			}
		}
