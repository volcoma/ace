...
{
    try_serialize(ar, ser20::make_nvp("left", obj.left));
    try_serialize(ar, ser20::make_nvp("top", obj.top));
    try_serialize(ar, ser20::make_nvp("right", obj.right));
    try_serialize(ar, ser20::make_nvp("bottom", obj.bottom));
}

template<typename Archive, typename T>
inline void SERIALIZE_FUNCTION_NAME(Archive& ar, size<T>& obj)
{
    try_serialize(ar, ser20::make_nvp("width", obj.width));
    try_serialize(ar, ser20::make_nvp("height", obj.height));
}
...
inline void SERIALIZE_FUNCTION_NAME(Archive& ar, range<T>& obj)
{
    ...
    try_serialize(ar, ser20::make_nvp("max", obj.max));
}
...
inline auto SAVE_MINIMAL_FUNCTION_NAME(Archive& ar, const hpp::uuid& obj) -> std::string
{
    ...
    //try_save(ar, ser20::make_nvp("data", hpp::to_string(obj)));
}
...
inline void LOAD_MINIMAL_FUNCTION_NAME(Archive& ar, hpp::uuid& obj, const std::string& suuid)
{
    ...
    //try_load(ar, ser20::make_nvp("data", suuid));
