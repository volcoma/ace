...
teapot_mesh_t::triangles_t::triangles_t(const teapot_mesh_t& mesh) noexcept
    : mesh_t{&mesh}
    , index_{0}
    , patch_mesh_{std::make_shared<bezier_mesh_t<4, 4>>(teapotData[0], gml::ivec2{mesh.segments_, mesh.segments_})}
...
void teapot_mesh_t::triangles_t::next()
{
    ...
    if(triangles_.done())
    {
        ...
        if(!done())
        {
            patch_mesh_ = std::make_shared<bezier_mesh_t<4, 4>>(teapotData[index_],
                                                                gml::ivec2{mesh_t->segments_, mesh_t->segments_});
...
void teapot_mesh_t::vertices_t::next()
{
    ...
    if(vertices_.done())
    {
        ...
        {
            patch_mesh_ = std::make_shared<bezier_mesh_t<4, 4>>(teapotData[index_],
                                                                gml::ivec2{mesh_->segments_, mesh_->segments_});
