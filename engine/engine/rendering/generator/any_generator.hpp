...
public:
    ...
    any_generator(generator gen) : base_{new derived<generator>{std::move(gen)}}
    {
    }

    any_generator(const any_generator& that) : base_{that.base_->clone()}
    {
    }
...
private:
    ...
    {
    public:
        explicit derived(generator gen) : generator_{std::move(gen)}
        {
        }
