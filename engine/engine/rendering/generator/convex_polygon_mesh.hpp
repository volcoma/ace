...
public:
    ...
    explicit convex_polygon_mesh_t(double radius = 1.0, int sides = 5, int segments = 4, int rings = 4) noexcept;

    //// @param vertices The corner vertex coordinates. Should form a convex polygon.
    explicit convex_polygon_mesh_t(const std::vector<gml::dvec2>& vertices, int segments = 1, int rings = 1) noexcept;

    /// @param vertices The corner vertex coordinates. Should be coplanar and
    /// form a convex polygon.
    /// calculated as an avarage.
    explicit convex_polygon_mesh_t(std::vector<gml::dvec3> vertices, int segments = 1, int rings = 1) noexcept;
