        ...
        if((search_flags & format_search_flags::floating_point) != 0u)
        {
            ...
            else if((search_flags & format_search_flags::two_channels) != 0u)
            {
                if(!requires_alpha)
                {
                    ...
                    else if(accept_padding && accept_half && is_format_supported(type_flags, texture_format::RGBA16F))
                    {
                        return texture_format::RGBA16F;
                    }
                    else if(accept_padding && accept_full && is_format_supported(type_flags, texture_format::RGBA32F))
                    {
                        return texture_format::RGBA32F;
                    }
            ...
            else if((search_flags & format_search_flags::one_channel) != 0u)
            {
                if(!requires_alpha)
                {
                    ...
                    else if(accept_padding && accept_half && is_format_supported(type_flags, texture_format::RG16F))
                    {
                        return texture_format::RG16F;
                    }
                    else if(accept_padding && accept_full && is_format_supported(type_flags, texture_format::RG32F))
                    {
                        return texture_format::RG32F;
                    }
                    else if(accept_padding && accept_half && is_format_supported(type_flags, texture_format::RGBA16F))
