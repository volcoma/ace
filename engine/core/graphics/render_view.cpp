...
auto render_view::get_texture(const std::string& id, uint16_t _width, uint16_t _height, bool _hasMips,
...
auto render_view::get_texture(const std::string& id, backbuffer_ratio _ratio, bool _hasMips,
							  uint16_t _numLayers, texture_format _format, uint64_t _flags) -> texture::ptr
{
	texture_key key{};
	uint16_t _width = 0;
	uint16_t _height = 0;
...
auto render_view::get_texture(const std::string& id, uint16_t _width, uint16_t _height, uint16_t _depth,
							  bool _hasMips, texture_format _format, uint64_t _flags, const memory_view* _mem)
	-> texture::ptr
{
	texture_key key{};
...
auto render_view::get_texture(const std::string& id, uint16_t _size, bool _hasMips, uint16_t _numLayers,
...
auto render_view::get_fbo(const std::string& id, const std::vector<texture::ptr>& bind_textures)
	-> frame_buffer::ptr
{
	fbo_key key{};
	key.id = id;
