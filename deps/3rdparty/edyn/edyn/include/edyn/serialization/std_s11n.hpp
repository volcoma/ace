...
void serialize(Archive &archive, std::vector<T> &vector) {
    using size_type = uint16_t;
    size_type size = size_type(std::min(vector.size(), static_cast<size_t>(std::numeric_limits<size_type>::max())));
    archive(size);
    vector.resize(size);
...
void serialize(Archive &archive, std::map<K, V> &map) {
    using size_type = uint16_t;
    size_type size = size_type(std::min(map.size(), static_cast<size_t>(std::numeric_limits<size_type>::max())));
    archive(size);
