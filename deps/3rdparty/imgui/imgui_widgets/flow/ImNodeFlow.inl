...
{
    return addIN_uid(name, name, tooltip, defReturn, std::move(filter), std::move(style));
}

template<typename T, typename U>
std::shared_ptr<InPin<T>> BaseNode::addIN_uid(const U& uid,
                                              ...
                                              std::shared_ptr<PinStyle> style)
{
    ...
    auto p = std::make_shared<InPin<T>>(h, name, tooltip, defReturn, std::move(filter), std::move(style), this, &m_inf);
...
template<typename T>
const T& BaseNode::showIN(const std::string& name,
                          const std::string& tooltip,
                          T defReturn,
                          std::function<bool(Pin*, Pin*)> filter,
                          std::shared_ptr<PinStyle> style)
{
    return showIN_uid(name, name, tooltip, defReturn, std::move(filter), std::move(style));
}

template<typename T, typename U>
const T& BaseNode::showIN_uid(const U& uid,
                              const std::string& name,
                              const std::string& tooltip,
                              T defReturn,
                              std::function<bool(Pin*, Pin*)> filter,
                              std::shared_ptr<PinStyle> style)
{
    ...
    m_dynamicIns.emplace_back(std::make_pair(
        1,
        std::make_shared<InPin<T>>(h, name, tooltip, defReturn, std::move(filter), std::move(style), this, &m_inf)));
...
std::shared_ptr<OutPin<T>> BaseNode::addOUT(const std::string& name,
                                            ...
                                            std::shared_ptr<PinStyle> style)
{
    return addOUT_uid<T>(name, name, tooltip, std::move(style));
}

template<typename T, typename U>
std::shared_ptr<OutPin<T>> BaseNode::addOUT_uid(const U& uid,
                                                const std::string& name,
                                                const std::string& tooltip,
                                                std::shared_ptr<PinStyle> style)
{
    PinUID h = std::hash<U>{}(uid);
    auto p = std::make_shared<OutPin<T>>(h, name, tooltip, std::move(style), this, &m_inf);
...
template<typename T>
void BaseNode::showOUT(const std::string& name,
                       const std::string& tooltip,
                       std::function<T()> behaviour,
                       std::shared_ptr<PinStyle> style)
{
    showOUT_uid<T>(name, name, tooltip, std::move(behaviour), std::move(style));
}

template<typename T, typename U>
void BaseNode::showOUT_uid(const U& uid,
                           const std::string& name,
                           const std::string& tooltip,
                           std::function<T()> behaviour,
                           std::shared_ptr<PinStyle> style)
{
    ...
    m_dynamicOuts.emplace_back(
        std::make_pair(2, std::make_shared<OutPin<T>>(h, name, tooltip, std::move(style), this, &m_inf)));
...
{
    PinUID h = std::hash<U>{}(uid);
    auto it = std::find_if(m_ins.begin(),
                           m_ins.end(),
                           [&h](std::shared_ptr<Pin>& p)
                           {
                               return p->getUid() == h;
                           });
...
{
    PinUID h = std::hash<U>{}(uid);
    auto it = std::find_if(m_outs.begin(),
                           m_outs.end(),
                           [&h](std::shared_ptr<Pin>& p)
                           {
                               return p->getUid() == h;
...
inline void Pin::update()
{
    ...
    if(ImGui::IsItemHovered())
    {
        ...
        if(!m_tooltip.empty())
        {
            ImGui::SetItemTooltip("%s", m_tooltip.c_str());
        }
    }
}
