...
// #define ConnectionFilter_SameType   [](ImFlow::Pin* out, ImFlow::Pin* in){ return out->getDataType() ==
// in->getDataType(); } #define ConnectionFilter_Numbers    [](ImFlow::Pin* out, ImFlow::Pin* in){ return
// out->getDataType() == typeid(double) || out->getDataType() == typeid(float) || out->getDataType() == typeid(int); }
...
struct PinStyleExtras
{
    ...
    float link_selected_outline_thickness = 5.5f;
    /// @brief Color of the outline of a selected link
    ImU32 outline_color = IM_COL32(80, 255, 0, 200);
...
public:
    ...
    ImU32 border_selected_color = IM_COL32(80, 255, 0, 200);
...
public:
    ...
    {
        return std::make_shared<NodeStyle>(IM_COL32(71, 142, 173, 255), ImColor(255, 255, 255, 255), 6.5f);
    }
    /// @brief <BR>Default green style
    static std::shared_ptr<NodeStyle> green()
    {
        return std::make_shared<NodeStyle>(IM_COL32(90, 191, 93, 255), ImColor(255, 255, 255, 255), 3.5f);
    }
    /// @brief <BR>Default red style
    static std::shared_ptr<NodeStyle> red()
    {
        return std::make_shared<NodeStyle>(IM_COL32(191, 90, 90, 255), ImColor(255, 255, 255, 255), 11.f);
    }
    /// @brief <BR>Default brown style
    static std::shared_ptr<NodeStyle> brown()
    {
        return std::make_shared<NodeStyle>(IM_COL32(191, 134, 90, 255), ImColor(255, 255, 255, 255), 6.5f);
    }
};
...
public:
     ...
     * @details Sets the content of a pop-up that can be displayed when dragging a link in the open instead of onto
     * another pin.
     ...
     */
    [[nodiscard]] bool getSingleUseClick() const
    {
     ...
     */
    [[nodiscard]] bool isNodeDragged() const
    {
     ...
     */
    void hoveredNode(BaseNode* hovering)
    {
     ...
     * @param hovering Pointer to the hovered link
     */
    void hoveredLink(std::weak_ptr<Link> hovering)
    {
        m_hoveredLink = hovering;
    }

    /**
     * @brief <BR>Check if currentty multi selecitng with rect
     */
    bool isRectSelecting() const
    {
        return m_isSelecting;
    }

    /**
     * @brief <BR>Returns the selection rect
     */
    ImRect getRectSelection() const
    {
...
private:
    ...
    ImVec2 m_selectStartPos{};
    ImVec2 m_selectEndPos{};
    ImRect m_selectionRect{};
};
 ...
 * @details Main class from which custom nodes can be created. All interactions with the main grid are handled
 * internally.
 */
class BaseNode
{
public:
    ...
    std::shared_ptr<InPin<T>> addIN_uid(const U& uid,
     ...
     *          The pin is created the first time showIN is called and kept alive as long as showIN is called each
     * frame. <BR> <BR> In this case the name of the pin will also be its UID. <BR> <BR> The UID must be unique only in
     * the context of the current node's inputs.
     * @tparam T Type of the data the pin will handle
     * @param name Name of the pin
     * @param defReturn Default return value when the pin is not connected
     * @param filter Connection filter
     ...
     */
    template<typename T>
    const T& showIN(const std::string& name,
                    const std::string& tooltip,
                    T defReturn,
                    std::function<bool(Pin*, Pin*)> filter,
                    std::shared_ptr<PinStyle> style = nullptr);
     ...
     *          The pin is created the first time showIN_uid is called and kept alive as long as showIN_uid is called
     * each frame. <BR> <BR> The UID must be unique only in the context of the current node's inputs.
     * @tparam T Type of the data the pin will handle
     * @tparam U Type of the UID
     * @param uid Unique identifier of the pin
     * @param name Name of the pin
     ...
     */
    template<typename T, typename U>
    const T& showIN_uid(const U& uid,
                        const std::string& name,
                        const std::string& tooltip,
                        T defReturn,
                        std::function<bool(Pin*, Pin*)> filter,
     ...
     * @details Must be called in the node constructor. WIll add an Output pin to the node with the given name and data
     * type. <BR> <BR> In this case the name of the pin will also be its UID. <BR> <BR> The UID must be unique only in
     ...
     * @brief <BR>Add an Output to the node
     * @details Must be called in the node constructor. WIll add an Output pin to the node with the given name and data
     * type. <BR> <BR> The UID must be unique only in the context of the current node's outputs.
     * @tparam T Type of the data the pin will handle
     * @tparam U Type of the UID
     * @param uid Unique identifier of the pin
     * @param name Name of the pin
     ...
     */
    template<typename T, typename U>
    [[nodiscard]] std::shared_ptr<OutPin<T>> addOUT_uid(const U& uid,
                                                        const std::string& name,
                                                        const std::string& tooltip,
                                                        std::shared_ptr<PinStyle> style = nullptr);
     ...
     *          The pin is created the first time showOUT is called and kept alive as long as showOUT is called each
     * frame. <BR> <BR> In this case the name of the pin will also be its UID. <BR> <BR> The UID must be unique only in
     * the context of the current node's outputs.
     * @tparam T Type of the data the pin will handle
     * @param name Name of the pin
     ...
     */
    template<typename T>
    void showOUT(const std::string& name,
                 const std::string& tooltip,
     ...
     * @brief <BR>Show a temporary output pin
     * @details Will show an output pin with the given name.
     *          The pin is created the first time showOUT_uid is called and kept alive as long as showOUT_uid is called
     * each frame. <BR> <BR> The UID must be unique only in the context of the current node's outputs.
     * @tparam T Type of the data the pin will handle
     * @tparam U Type of the UID
     * @param uid Unique identifier of the pin
     * @param name Name of the pin
     ...
     * @details Get a reference to the value of an input pin, the value is stored in the output pin at the other end of
     * the link.
     * @tparam T Data type
     * @tparam U Type of the UID
     * @param uid Unique identifier of the pin
     ...
     */
    const std::vector<std::shared_ptr<Pin>>& getIns()
    {
        return m_ins;
    }
     ...
     * @return Const reference to node's internal list
     */
    const std::vector<std::shared_ptr<Pin>>& getOuts()
    {
        return m_outs;
    }
     ...
     */
    [[nodiscard]] bool toDestroy() const
    {
        return m_destroyed;
    }
     ...
     */
    [[nodiscard]] NodeUID getUID() const
    {
        return m_uid;
    }
     ...
     */
    const std::shared_ptr<NodeStyle>& getStyle()
    {
        return m_style;
    }
...
public:
     ...
     */
    virtual std::weak_ptr<Link> getLink()
    {
     ...
     */
    [[nodiscard]] const ImVec2& getSize()
    {
        return m_size;
    }
     ...
     */
    std::shared_ptr<PinStyle>& getStyle()
    {
        return m_style;
    }
...
public:
    ...
    static std::function<bool(Pin*, Pin*)> Numbers()
    {
        return [](Pin* out, Pin* in)
        {
            return out->getDataType() == typeid(double) || out->getDataType() == typeid(float) ||
...
public:
    ...
    explicit InPin(PinUID uid,
                   ...
                   BaseNode* parent,
                   ImNodeFlow** inf)
        : Pin(uid, name, tooltip, style, PinType_Input, parent, inf)
     ...
     */
    void allowSameNodeConnections(bool state)
    {
        m_allowSelfConnection = state;
    }
     ...
     */
    std::weak_ptr<Link> getLink() override
    {
        return m_link;
    }
...
public:
     ...
     * @param style Style of the pin
     */
    explicit OutPin(PinUID uid,
                    const std::string& name,
                    const std::string& tooltip,
                    std::shared_ptr<PinStyle> style,
                    BaseNode* parent,
                    ImNodeFlow** inf)
        : Pin(uid, name, tooltip, style, PinType_Output, parent, inf)
    {
    }
     ...
     * @details Used to define the pin behaviour. This is what gets the data from the parent's inputs, and applies the
     * needed logic.
