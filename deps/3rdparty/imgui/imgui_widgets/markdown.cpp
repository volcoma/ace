...
void Markdown(const char* markdown_)
{
    ...
    Markdown(markdown_, int32_t(strlen(markdown_)), config);
}
            ...
            case Link::HAS_SQUARE_BRACKETS_ROUND_BRACKET_OPEN:
                if(c == ')') // it's a link, render it.
                {
                    ...
                    if(ImGui::IsItemHovered())
                    {
                        ...
                        ImGui::UnderLine(style.Colors[ImGuiCol_Text]);
                        ImGui::SetMouseCursor(ImGuiMouseCursor_Hand);
                        if(mdConfig_.linkTooltip)
                        {
                    ...
                    {
                        ImGui::UnderLine(style.Colors[ImGuiCol_TextDisabled]);
                    }
                    ImGui::SameLine(0.0f, 0.0f);
