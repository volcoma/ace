...
macro (enable_sanitizer_flags sanitize_option)
    if (${sanitize_option} MATCHES "address")
        ...
        if (CMAKE_CXX_COMPILER_ID MATCHES "MSVC")
            ...
            set(XSAN_LINK_FLAGS "/fsanitize=address")
        else()
            set(XSAN_COMPILE_FLAGS "-fsanitize=address -fno-omit-frame-pointer -fno-optimize-sibling-calls")
            set(XSAN_LINK_LIBRARIES "asan")
        endif()
    elseif (${sanitize_option} MATCHES "thread")
        check_compiler_version("4.8" "3.1" "99.99")
        set(XSAN_COMPILE_FLAGS "-fsanitize=thread")
        set(XSAN_LINK_LIBRARIES "tsan")
    ...
    message(STATUS "enable_sanitizers_flags")
    message(STATUS "CMAKE_CXX_FLAGS=${XSAN_COMPILE_FLAGS}")
    message(STATUS "CMAKE_LINK_FLAGS=${XSAN_LINK_FLAGS}")
    message(STATUS "XSAN_LINK_LIBRARIES=${XSAN_LINK_LIBRARIES}")
endmacro ()

# Define a function that enables sanitizers based on the current compiler and ECM_ENABLE_SANITIZERS list
macro(enable_sanitizers_for_compiler)
    if (ECM_ENABLE_SANITIZERS)
        if (CMAKE_CXX_COMPILER_ID MATCHES "GNU" OR CMAKE_CXX_COMPILER_ID MATCHES "Clang" OR CMAKE_CXX_COMPILER_ID MATCHES "MSVC")
            # Iterate over each sanitizer in the ECM_ENABLE_SANITIZERS list
            foreach (CUR_SANITIZER ${ECM_ENABLE_SANITIZERS})
                # Convert to lowercase
                string(TOLOWER ${CUR_SANITIZER} CUR_SANITIZER)
                # Enable appropriate flags based on sanitizer option
                enable_sanitizer_flags(${CUR_SANITIZER})

                # Add flags based on the compiler being used
                if (CMAKE_C_COMPILER_ID MATCHES "Clang")
                    set(CMAKE_C_FLAGS "${CMAKE_C_FLAGS} ${XSAN_COMPILE_FLAGS}")
                endif()
                set(CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} ${XSAN_COMPILE_FLAGS}")
                set( CMAKE_LINK_FLAGS "${CMAKE_LINK_FLAGS} ${XSAN_LINK_FLAGS}" )

                # Link libraries if GCC is the compiler
                if (CMAKE_CXX_COMPILER_ID MATCHES "GNU")
                    link_libraries(${XSAN_LINK_LIBRARIES})
                endif()

                # Special handling for Clang
